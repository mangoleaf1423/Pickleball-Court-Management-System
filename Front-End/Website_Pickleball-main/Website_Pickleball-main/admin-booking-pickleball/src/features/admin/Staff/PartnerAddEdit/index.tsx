...
const PartnerAddEdit = () => {
  ...
  const [showCourtField, setShowCourtField] = useState(false);
    ...
    try {
      ...
      if (!id) {
        ...
        const updatePayload = {
          ...
          courtIds: formValues.courtIds || null,
          active: true
        };
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        ...
        // Lọc bỏ quyền ADMIN từ danh sách quyền
        const filteredRoles = res.data.result.filter((role: any) => role.name !== 'ADMIN');
  ...
  useEffect(() => {
    const fetchUserData = async (userId: string) => {
      try {
        ...
        if (res?.result) {
          const roles = res.result.roles?.map((role) => role.name);
          setSelectedRoles(roles);
          setShowCourtField(roles.includes('STAFF'));
  ...
  const handleRoleChange = (value: string) => {
    // Chuyển đổi giá trị đơn thành mảng chứa một phần tử
    const values = [value];
    setSelectedRoles(values);
    setShowCourtField(values.includes('STAFF'));
    
    // Nếu không có vai trò STAFF, xóa giá trị courtIds
    if (!values.includes('STAFF')) {
      form.setFieldsValue({ courtIds: undefined });
    }
  };
                ...
                {showCourtField && (
                  <Col span={12}>
                    <Form.Item 
                      ...
                      name="courtIds" 
                      rules={[{ required: selectedRoles.includes('STAFF'), message: 'Vui lòng chọn sân!' }]}
                    >
                      <Select
                        mode="multiple"
                        showSearch
